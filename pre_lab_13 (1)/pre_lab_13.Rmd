...
We'll be using a service called Groq for the examples here. You should [sign up for a free account](https://w1ww.groq.com) and [create an API key](https://console.groq.com/docs/quickstart). Make sure you copy that key. We'll also need to install an R package to handle the responses:
...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
**The second 10 names seem to be a bit more complicated, but I don't see much of a difference. **
...
**It looks like llama got a decent amount, but not everything. **
...
**It looks like Llama got it right - I would change the prompt to isolate the key duties more. **
...
**Yes, because it explains the error message and tells me how to fix it. **
...
**Yeah, I think it would help me understand the data. The output makes sense - most of the ways have proper context. All the assumptions seem correct. **
...
**It did pretty well - I would change being as specific to making assumptions about the context, just to see what it says. **
...
**It didn't load, so I got no results. Just gave me an error after spinning for a few minutes. ** 
