...
There are 1,640 problems. I think the problem that R is describing is unspecific column types. 
...
It doesn't show any parsing errors, I don't think. 
...
The district is a double precision data type, while the precinct is a character data type. 
```{r}
...
402,345 where mail is NA, meaning that the votes weren't mail ones. 
...
In the data for January 1900 there are 12 registrations - I think the first 20 results suggest that there wasn't a lot of voters registered in Yakdin for a while. 
...
Legacy data - I'm unfamiliar with the term. 
...
The function put an X in front of it. 
...
I think the error, which says there's an invalid type of argument, means that there's a value there that doesn't work with the function. 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. one duplicate, donor name Derrick Hamilton
...
It now stops after the fifth letter. 
