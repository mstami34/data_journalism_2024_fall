...
```{r}
MD_absentee_ballots_sent_and_returned <- read_csv("data/md_absentee_ballots.csv")
MD_absentee_ballots_by_date <- read_csv("data/md_absentee_ballots_date.csv")
```
...
**A1.** Worcester County has the highest percentage of total absentee ballots returned of total ballots sent. 
``` {r}
MD_absentee_ballots_sent_and_returned <- MD_absentee_ballots_sent_and_returned |> 
  mutate(Ballot_Percentage = (`TOTAL RECEIVED`/ `TOTAL SENT`) *100 )

updated_ballots <- MD_absentee_ballots_sent_and_returned |>
  arrange(desc(Ballot_Percentage)) |>
  select(`COUNTY NAME`, Ballot_Percentage) |> 
  head(10)
...
updated_ballots |>
  ggplot() +
  geom_bar(aes(x=`COUNTY NAME`, weight = Ballot_Percentage )) +
  theme_clean() +
  coord_flip() +
    labs(
      title = "Percentage of total absentee ballots returned of total ballots sent", 
      x = "County",
      y = "Percentage",
...
**A2.**
The ten counties with the lowest percentage are Baltimore County, Montgomery, Prince Georges, Frederick, Baltimore County, Charles, Howard, Cecil, Anne Arundel, and Carroll. In most of these counties, democrats are returning more than republicans. 
```{r}
MD_absentee_ballots_sent_and_returned <- MD_absentee_ballots_sent_and_returned |> 
  mutate(Dem_Ballot_Percentage = (`DEM RECEIVED`/ `DEM_SENT`) *100 )

MD_absentee_ballots_sent_and_returned <- MD_absentee_ballots_sent_and_returned |> 
  mutate(Rep_Ballot_Percentage = (`REP RECEIVED`/ `REP SENT`) *100 )

DEM_updated_ballots <- MD_absentee_ballots_sent_and_returned |>
  arrange((Dem_Ballot_Percentage)) |>
  select(`COUNTY NAME`, Dem_Ballot_Percentage) |> 
  head(10)
DEM_updated_ballots

REP_updated_ballots <- MD_absentee_ballots_sent_and_returned |>
  arrange((Rep_Ballot_Percentage)) |>
  select(`COUNTY NAME`, Rep_Ballot_Percentage) |> 
  head(10)
REP_updated_ballots
...
DEM_updated_ballots |>
  ggplot() +
  geom_bar(aes(x=`COUNTY NAME`, weight = Dem_Ballot_Percentage )) +
  theme_clean() +
  coord_flip() +
    labs(
      title = "Percentage of ballots returned by Democrats", 
      x = "County",
      y = "Percentage",
      caption = "Source: Maryland State Board of Elections"
    )
...
**A3** Democrats are voting a lot more than the other two, which matches what I know about voting habits. 

```{r}
MD_absentee_ballots_by_date |>
...
**A4.** 
There's a pretty clear correlation between total sent and total received for every county - it really doesn't look like there are many outliers. All counties seem to be performing in line with the overall trend. 
```{r}
MD_absentee_ballots |>
  ggplot() +
  geom_point(aes(x=`TOTAL SENT`, y=`TOTAL RECEIVED`)) +
  geom_text(aes(x=`TOTAL SENT`, y=`TOTAL RECEIVED`, label = `COUNTY NAME`), check_overlap = TRUE) +
geom_smooth(aes(x=`TOTAL SENT`, y=`TOTAL RECEIVED`))
