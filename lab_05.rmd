...
```{r}
harris_campaign <- read_csv("data/august_harris.csv")
harris_campaign_p2 <- read_csv("data/july_harris.csv")

harris_campaign_combined <- bind_rows(harris_campaign, harris_campaign_p2)

glimpse(harris_campaign_combined)
...
A1. I used these columns because they're shared on both sides, so it works well. I don't see any problems with this join. 
...
```{r}
new_voters_after_july_20 <- new_voters |>
  setorder(date > mdy("07-20-2024"))
new_voters_after_july_20 |>
...
A3. Slightly more females than males. 
...
A4:
How many donations has harris gotten from non-democrat males? To answer this, I would have to create a new group of harris's donations that exclude democrats, and then sort by gender. 