...
**A1.**
Both campaigns are spending a ton of money on advertising, salary, and fundraising consulting. 
```{r}


md_new_senate_expenses|>
  rename(purpose_original = purpose) |>
  rename(purpose = purpose_clean) |>
  ...
  mutate(purpose = str_remove(purpose, "EXPENSE")) |>
  group_by(purpose) |>
...
**A2.**
Alsobrooks spent a significant amount more than Hogan. Both of their largest categories were Ads and staffing, but her numbers dwarfed his. 
...
**A3.**
Alsobrooks' spending involving media advertising has grown significantly over time. The amount grew pretty signficantly from december to march, then massively spiked in april, eventually coming back down to numbers similar to March in May. 
```{r}
md_new_senate_expenses |>
  filter(candidate == "Alsobrooks", 
         purpose == "Media Advertising"
         ) |>
...
**A4.** Alsobrooks' salary spending is 822,015, while Hogan's staff spending is 285,672.8. Both parties have a singular name for spending, although Hogan's is under "staffing - see memo" and Alsobrooks' is under "salary". 

```{r}
md_new_senate_expenses |>
  filter(purpose_clean == "STAFFING - SEE MEMO", candidate == "Hogan") |>
  group_by(candidate, purpose) |>
  summarise(contributions = n(), 
            amount = sum(amount)) |>
  arrange(desc(amount))
...
**A5**
Probably Alsobrooks' sudden jump in funds in April. Look around that time for events and circumstances that coincided with it. 
