...
```{r echo=FALSE}
census_api_key("a08a68ac1c28962a796c651e815d67c03fa36d1f", overwrite=TRUE)
```
...
median_age_by_county <- get_acs(geography = 'county', variables = (median_age ='B01002_001'), state = "MD", year = 2022, survey = "acs5"
)
median_age_by_county
...
**A2**
Talbot and Worchester counties have a median age of at least 50. 
...
frederick_joined <- frederick_cvr |>
...
**A3**
11,871 voted for different parties in the governor and comptroller's race. 95,221 voted rot the same party in both races. 16.2 percent of all voters split their tickets. 89916 voters chose the same party in all five contests - good for 83.8 percent. 
```{r}
frederick_joined |>
...
frederick_joined |>
  mutate(split_vote = case_when(
    governor_lt_governor == comptroller & comptroller == attorney_general &
    attorney_general == u_s_senator & u_s_senator == representative_in_congress_6 ~ "voted the same in all five", 
    TRUE ~ "split vote"
  )) |>
  group_by(split_vote) |>
  summarise(votes = n()) |>
...
**A4**
21702 has the most voters who preferred rep in every race except the governor's. 
```{r} 
frederick_joined |>
  mutate(split_vote = case_when(
    u_s_senator == "REP" & representative_in_congress_6 == "REP" & comptroller == "REP" & attorney_general == "REP" & governor_lt_governor == "DEM" ~ "split republican",
    TRUE ~ "all others"
  )) |>
  filter(split_vote == "split republican") |>
  group_by(zipcode) |>
  summarise(votes = n()) |>
  arrange(desc(votes))
