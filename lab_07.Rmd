...
frederick_joined |>
  mutate(split_vote = case_when(
    #governor_lt_governor == comptroller & comptroller == attorney_general & attorney_general == u_s_senator & u_s_senator == representative_in_congress_6 ~ "voted the same in all 5", 
    u_s_senator = "REP" & representative_in_congress_6 = "REP" & comptroller = "COMP" & attorney_general == "REP" & governor_lt_governor = "DEM" ~ "split republican"
    TRUE ~ "split vote"
  )) |>
  group_by(zipcode, split_vote) |>
  summarise(votes = n())
  arrange(desc(votes))
