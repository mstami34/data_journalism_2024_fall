...
Total_McMullin_Votes
  ...
  mutate(Difference_McMullin_v_Wilson = Total_McMullin_Votes - Total_Wilson_Votes)
...
Total_Romney_Votes
  ...
  Total_Lee_Votes
   ...
   joined_data_both <- joined_data_both |>
  mutate(Difference_Romney_v_Lee = Total_Romney_Votes - Total_Lee_Votes)
```
...
dates <- page %>%
  ...
  html_text() %>%
  trimws() # Clean any leading/trailing whitespace
...
# Step 3: Combine the extracted data into a dataframe
press_releases_df <- data.frame(
  date = dates,
  title = titles,
  url = urls,
...
press_releases_df$date <- mdy(press_releases_df$date) # Assuming date is in "MM/DD/YYYY" format

# Rename columns
colnames(press_releases_df) <- c("Date", "Title", "URL")

# Step 5: Display the dataframe
head(press_releases_df)
