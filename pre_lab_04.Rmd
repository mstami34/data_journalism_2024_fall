...
**Task** Run the following code to load some campaign expenditure data focused on food-related expenses in Washington, D.C. -RUN
...
Some recipients are inserted differently, even though they're the same. 
...
Capitalization changes and different formatting
...
Grouped the different Millar & Co's, grouped the hiltons together, made the Walmart all caps. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **No**
